...
{
    public partial class Newmail_Form : Form
    {
        ...
        private void Newmail_Form_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //นับจำนวนรายการ
                connection.Open();
                string query1 = "SELECT COUNT(*) as mycount FROM about_email WHERE title_type = 'N';";
                ...
                connection.Open();
                cmd2.CommandText = "SELECT MIN(getinfo_date) AS from_date FROM about_email WHERE title_type = 'N'";
            ...
            try
            {
                ...
                c1.gridview("select * from about_email WHERE title_type = 'N'", dt);
        ...
        private void Newmail_Form_Shown(object sender, EventArgs e)
        {
            //ลบ highlight เริ่มต้น
            dataGridView1.ClearSelection();
        }
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (textBox1.Text == "")
                {
                    condition = "where title_type = 'N' and getinfo_date between #" + newfrom + "# and #" + newto + "#";
                }
                else
                {
                    ...
                    if (datatype == "ทั้งหมด")
                    {
                        ...
                        condition += " or status like '%" + stext + "%')";
                    }
                    ...
                    else
                    {
                        ...
                        condition = "where title_type = 'N' and " + datatype + " like '%" + textBox1.Text + "%' and getinfo_date between #" + newfrom + "# and #" + newto + "#";
                    }
                }
                //เคลียค่าในตาราง
                dt.Rows.Clear();
        ...
        {
            Add_newmail m2 = new Add_newmail();
            m2.Show();
        }
        ...
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                Edit_newmail f4 = new Edit_newmail(Pid);
                f4.Show();
            }
        }
        ...
        private void print_Click(object sender, EventArgs e)
        {
            if (dataGridView1.Rows.Count > 0)
            {
                ...
                sfd.FileName = "สมัคร E-mail ใหม่.pdf";       //ชื่อไฟล์เริ่มต้น
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (!fileError)
                    {
                        try
                        {
                            ...
                            PdfPTable pdfTable = new PdfPTable(17);     //จำนวนคอลัมน์
                            pdfTable.DefaultCell.Padding = 3;
                            pdfTable.WidthPercentage = 100;
                            pdfTable.HorizontalAlignment = Element.ALIGN_LEFT;

                            //กำหนดขนาดแต่ละคอลัมน์
                            float[] colWidths = new float[17];
                            colWidths[0] = 5f;
                            colWidths[1] = 4f;
                            colWidths[2] = 9f;
                            colWidths[3] = 9f;
                            ...
                            PdfPCell cell;
                            //หัวกระดาษ
                            p = new Phrase("สมัคร E-mail ใหม่สำหรับข้าราชการบรรจุใหม่ยังไม่มี E-mail", ftitle);
                            cell = new PdfPCell(p);
                            cell.Colspan = 17;          //ขนาดเท่ากับจำนวนคอลัมน์
                            cell.HorizontalAlignment = Element.ALIGN_CENTER;
                            cell.Border = iTextSharp.text.Rectangle.NO_BORDER;
                            cell.PaddingBottom = 6;
                            pdfTable.AddCell(cell);

                            p = new Phrase("และขอใช้งาน Internet ภายในโรงเรียนนายเรืออากาศนวมินทกษัตริยาธิราช", ftitle);
                            cell = new PdfPCell(p);
                            cell.Colspan = 17;
                            cell.HorizontalAlignment = Element.ALIGN_CENTER;
                            cell.Border = iTextSharp.text.Rectangle.NO_BORDER;
                            ...
                            cell = new PdfPCell(new Phrase("หมายเลข ประชาชน", fnt));
                            cell.HorizontalAlignment = Element.ALIGN_CENTER;
                            cell.PaddingBottom = 6;
                            pdfTable.AddCell(cell);

                            cell = new PdfPCell(new Phrase("หมายเลข ข้าราชการ", fnt));
                            cell.HorizontalAlignment = Element.ALIGN_CENTER;
                            cell.PaddingBottom = 6;
                            pdfTable.AddCell(cell);

                            cell = new PdfPCell(new Phrase("วัน/เดือน/ปี เกิด", fnt));
                            cell.HorizontalAlignment = Element.ALIGN_CENTER;
                            cell.PaddingBottom = 6;
                            pdfTable.AddCell(cell);
                            ...
                            cell = new PdfPCell(new Phrase("เบอร์โทรภายใน", fnt));
                            cell.HorizontalAlignment = Element.ALIGN_CENTER;
                            cell.PaddingBottom = 6;
                            pdfTable.AddCell(cell);
                            ...
                            foreach (DataGridViewRow row in dataGridView1.Rows)
                            {
                                ...
                                cell = new PdfPCell(new Phrase(row.Cells["people_id"].Value == null ?
                                    string.Empty : row.Cells["people_id"].Value.ToString(), fdata));
                                cell.HorizontalAlignment = Element.ALIGN_CENTER;
                                cell.PaddingBottom = 6;
                                pdfTable.AddCell(cell);

                                cell = new PdfPCell(new Phrase(row.Cells["gov_id"].Value == null ?
                                    string.Empty : row.Cells["gov_id"].Value.ToString(), fdata));
                                cell.HorizontalAlignment = Element.ALIGN_CENTER;
                                cell.PaddingBottom = 6;
                                pdfTable.AddCell(cell);
                                ...
                                cell = new PdfPCell(new Phrase(birthdate == null ?
                                    string.Empty : birthdate.ToString("d/M/yyyy"), fdata));
                                cell.HorizontalAlignment = Element.ALIGN_CENTER;
                                cell.PaddingBottom = 6;
                                pdfTable.AddCell(cell);
